# Redline — installable app

This folder is a complete, self-contained copy of Redline that can be installed
as a real app on Windows and Android, with full offline support. Everything it
needs (the PDF export/import libraries and fonts) is bundled locally — nothing
is loaded from the internet once it's running.

**Why a separate download?** The Redline link you already have (on claude.ai)
can't be turned into an installable app — that platform doesn't allow a page to
register the pieces (a "manifest" and a background "service worker") that
Windows/Android require before they'll offer an Install button. This bundle is
the same Redline, packaged so a real web host CAN offer that Install button.
...
2. Click the **+** in the top right → **New repository**. Name it anything,
   e.g. `redline-app`. Keep it **Public**. Click **Create repository**.
...
6. Wait about a minute, then refresh that Pages settings screen — it will
   ...
   `https://your-username.github.io/redline-app/`

That URL is your permanent link to Redline. Open it once while online so it
...
3. Confirm. Redline now opens in its own window from the Start menu, with no
   browser bar, and works fully offline.

**On Android (Chrome):**
1. Open your hosted URL in Chrome.
2. Tap the **⋮** menu → **Install app** (or **Add to Home screen**).
3. Confirm. Redline now appears as a real app icon on your home screen and
   launches full-screen, no browser bar, and works fully offline.

## Updating it later

If you ever want changes made to Redline, come back and ask — I'll rebuild
